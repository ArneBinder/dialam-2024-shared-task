...
class evaluation:
    ...
    def matching_calculation(
        ...
        """
        # Graph construction.
        graph1, graph2 = match.get_graphs(predicted_data, gold_data)
        # Creating similarity matrix for propositional relations.
        prop_rels = match.get_sim_matrix(graph1, graph2, "propositions")
        # Creating similarity matrix for locutional relations.
        loc_rels = match.get_sim_matrix(graph1, graph2, "locutions")
        # (1) Do we have the same YA > S transitions? Do their annotations coincide?
        # Anchoring on S-nodes (RA/CA/MA) and combining them (checking how many YA-anchors, predecessors of S-nodes, have the same/different text field annotations).
        ra_a = match.s_rel_anchor("RA", graph1, graph2)
        ma_a = match.s_rel_anchor("MA", graph1, graph2)
        ca_a = match.s_rel_anchor("CA", graph1, graph2)
        all_a = match.combine_s_node_matrix(ra_a, ca_a, ma_a)
        all_s_a_dict = match.convert_to_dict(all_a)
        # (2) Do we have the same S-nodes?
        # Comparing propositional relations, building a confusion matrix for S-node (RA, MA, CA) matches between the two graphs.
        ...
        # Getting all YAs anchored in locutions, comparing the text field annotations of YA-nodes, successors of L-nodes, anchored in locutions.
        ...
        # Getting all YAs in propositions, comparing the text field annotations of YA-nodes, predecessors of I-nodes, anchored in propositions.
        ...
        # Getting all TAs anchored in locutions, checking whether each L-node pair (from graph1 and graph2) has the same amount of outgoing TA-nodes.
        ...
        # Getting all YAs anchored in propositions, comparing the text field annotations of L-/TA-nodes that are predecessors of YA-nodes anchored in propositions (I-nodes).
